...
struct fenwick_tree {
    ...
    void apply(int i, const value_type &val) {
        for (++i; i <= n; i += i & -i) {
            ft[i] = groupoid_type::op(ft[i], val);
        }
    }
